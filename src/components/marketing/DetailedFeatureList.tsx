...
export function DetailedFeatureList({ keyFeatures }: DetailedFeatureListProps) {
  return (
    <div>
      <h2 className="text-2xl font-bold font-lato mb-6">Key Capabilities</h2>
      <motion.div 
        className="space-y-6"
        initial="hidden"
        whileInView="visible"
      ...
      >
        {keyFeatures.map((point) => (
          ...
          >
            ...
            <div>
              <h4 className="font-semibold font-lato text-gray-900">{point.title}</h4>
              <p className="mt-1 text-sm font-roboto text-muted-foreground leading-relaxed">{point.description}</p>
            </div>
          </motion.div>
        ))}
      </motion.div>
